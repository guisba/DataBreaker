...
const GENERATED_FIELDS=new Set([
  'Now','NewGUID','NewUUID','FileSequence','ProcessingTime'
]);
...
function exifCategory(rawName,short,group){
  ...
  if(GENERATED_FIELDS.has(short))return 'generated/runtime metadata';
...
function exifImpact(rawName,short,group){
  const all=`${rawName} ${short} ${group}`.toLowerCase();
  if(GENERATED_FIELDS.has(short))return 'low';
...
function exifExplanation(rawName,short,group){
  if(GENERATED_FIELDS.has(short)){
    return 'Value generated by ExifTool at scan time. It is shown for completeness but is not embedded source-file metadata.';
  }
